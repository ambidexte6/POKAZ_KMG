...
#с1.гаснет .слой:not(.база),#с1.гаснет .бас_список li,#с1.гаснет .легенда,#с1.гаснет svg.слой path,#с1.гаснет svg.слой text{opacity:0!important;transition:opacity .8s ease!important}
...
.список_проектов .действ span b{background:#7DB8FF}.список_проектов .кмг span b{background:#D9A6FF}.список_проектов .партн span b{background:#FFE0B8}.список_проектов .гин span b{background:var(--мята)}
...
<section class="слайд виден" id="с1" aria-label="Осадочные бассейны Казахстана">
  ...
  <div class="полотно">
    <div class="бас_сетка">
      ...
      <ul class="бас_список">
{li_т('15', 'бассейнов', 'осадочных бассейнов на территории Казахстана — контуры на карте пунктиром', 1)}{li_т('6', 'бассейнов', 'малоизученные: Тенизский, Балхашский, Илийский, Зайсанский, Алакольский, Северо-Казахстанский', 2, 'мало')}{li_т('4', 'бассейна', 'с доказанной нефтегазоносностью: Прииртышский, Северо-Торгайский, Аральский, Сырдарьинский', 3, 'мнб')}{li_т('5', 'моделей', '<em>цифровые бассейновые модели</em>: Прикаспийский, Устюрт-Бозашинский, Мангышлакский, Южно-Торгайский, Шу-Сарысуский — структурные карты и очаги генерации УВ', 4, 'пять')}      </ul>
...
проекты = [пр for пр in портфель['проекты'] if пр['класс'] != 'жылыой']   # контур Жылыоя — не проект, штрих-рамка
...
def свг_участки():
    ...
    for пр in проекты:
        x, y = пр['центр']; dx, dy = СДВИГ_НОМЕРА.get(пр['имя'], (0, 0))
...
def список_проектов():
    ...
    s = ''
    for колонка in (['действ', 'гин'], ['кмг', 'партн']):
        s += '        <div class="колонка">\n'
        for кл in колонка:
...
<section class="слайд" id="с4" aria-label="Программа ГРР до 2030 года">
  ...
  <div class="полотно">
    <div class="бас_сетка программа">
      <div class="левая">
        <ul class="бас_список">
{li_0('23', 'проекта', 'на разных стадиях подготовки · <em>26</em> поисковых скважин, проходка 89 тыс. м', 'действ')}{li_0('2D · 3D', '', 'сейсморазведка <em>6 250 км</em> и <em>2 540 км²</em> · региональная 2D по Прикаспию — 60 тыс. км', 'партн')}{li_0('4,7', 'млрд т у.т.', 'потенциальная геологическая ресурсная база программы', 'пять')}        </ul>
...
(function(){
  ...
  var с1 = document.getElementById("с1"), такты = [].slice.call(с1.querySelectorAll("[data-такт]")), ВСЕГО = 4;
  var ШАГ = 2000, ПАУЗА = 6000, ГАШЕНИЕ = 900, таймер = null;
  ...
  function цикл(){
    ...
    (function шаг(){
      ...
      else { таймер = setTimeout(function(){ с1.classList.add("гаснет"); таймер = setTimeout(function(){ с1.classList.remove("гаснет"); цикл(); }, ГАШЕНИЕ); }, ПАУЗА); }
    })();
  }
